...
function displayData(data) {
    ...
    data.forEach(item => {
        const productElement = productMaker(item.name, item.image,item.address, item.bio,  item.price, item.rating, item.experience,item.Rank,item._id);
...
//  card 
function productMaker( image,name,address,profession,bio,Rank,experience,rating,_id) {
    // let best = "";
    // let star = ""
    ...
    <div class="image-container"><img class="product_img" src=${name} alt=""></div>
    <h4 class="title"> ${image}</h4>
     <p class="rating">Address : ${address}</p>
    <h2 class="price"> ${bio} </h2>

    <p class="brand">Rank : ${rating}</p>
    <p class="category">Rating : ${Rank}</p>

    <p class="material"> Experience : ${experience} </p
   ...
   // console.log('i am invoked ', numOfPages)
    ...
    for (let btn of paginationButtons) {
      btn.addEventListener('click', (e) => {
        ...
        fetchdata(`?gender=Male&page=${+dataId}&limit=3`);
      })
    }
}
